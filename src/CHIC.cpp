...
CHIC::CHIC(std::string_view mode, double theta_12, double theta_23,
           double theta_13, double delta_cp, double dm2_21, double dm2_31,
           double density, double Y_e) {
  ...
  // Apply values for vacuum Hamiltonian
  ...
  // Apply values for matter Hamiltonian
  update_Ye(Y_e);
...
void CHIC::_compute_hamiltonians() {
  ...
  if (TrHs2 < EPSILON) {
    throw std::invalid_argument("Eigenvalues are (almost) degenerate, please "
                                "check your input parameters.");
  } else {
    lambdas[2] = std::sqrt(OptConstants::TWO_THIRDS * TrHs2) *
                 std::cos(std::acos(DetHs * std::sqrt(54.0 / trHs2_cubed)) *
                          OptConstants::INV_3);
    lambdas[1] = -(0.5 * lambdas[2] - std::sqrt(0.25 * lambdas[2] * lambdas[2] -
                                                DetHs / lambdas[2]));
...
CHICDIFF::CHICDIFF(std::string_view mode, double theta_12, double theta_23,
                   double theta_13, double delta_cp, double dm2_21,
                   double dm2_31, double density, double Y_e)
    : CHIC(mode, theta_12, theta_23, theta_13, delta_cp, dm2_21, dm2_31,
           density, Y_e) {}

Eigen::Matrix3d
CHICDIFF::compute_oscillations_derivatives(std::string_view param, double E,
                                           double L) {
  if (need_update) {
    _set_matrices();
    E0 = E;
...
void CHICDIFF::_set_dHs() {
  ...
  if (param0 == "density") {
    ...
    throw std::invalid_argument(std::string("Unknown parameter: ") +
                                std::string(param0));
  }
  if (param0 != "density")
    dHs = inv_2E * dHs;
...
void CHICDIFF::_amplitude_derivative() {
  ...
  I_ijk(0, 1) = I_ijk(1, 0) =
      (diff_lambdas[0] * exp_lambdas[1] - diff_lambdas[1] * exp_lambdas[0]) /
      (lambdas[1] - lambdas[0]);
  I_ijk(0, 2) = I_ijk(2, 0) =
      (diff_lambdas[2] * exp_lambdas[0] - diff_lambdas[0] * exp_lambdas[2]) /
      (lambdas[0] - lambdas[2]);
  I_ijk(1, 2) = I_ijk(2, 1) =
      (diff_lambdas[1] * exp_lambdas[2] - diff_lambdas[2] * exp_lambdas[1]) /
      (lambdas[2] - lambdas[1]);
...
void CHICDIFF::dHs_drho() {
  ...
  dHs.diagonal().array() -=
      flip * OptConstants::WEAK * Y_e * OptConstants::INV_3;
}
