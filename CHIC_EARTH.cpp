...
// --- Protected injection constructor (used by derived classes) ---
...
// --- Public constructor (builds a plain CHIC and delegates) ---
...
void CHICEARTH::_compute_J_layer(int i_layer, bool is_deepest) {
  if (is_deepest)
    iL = std::complex<double>(0.0, -2.0 * tracks[i_layer] * OptConstants::BASELINE_FACTOR);
  else
    iL = std::complex<double>(0.0, -std::abs(tracks[i_layer] - tracks[i_layer-1]) * OptConstants::BASELINE_FACTOR);
...
void CHICEARTH::_update_J(bool is_deepest) {
  if (is_deepest) J = J_layer;
  else            J = J_layer * J * J_layer;
}

void CHICEARTH::_layer_amplitude(int i_layer, bool is_deepest) {
  _compute_J_layer(i_layer, is_deepest);
  _update_J(is_deepest);
}
...
void CHICEARTHDIFF::_layer_amplitude_diff(int i_layer, bool is_deepest) {
  ...
  cdJ_diag[0] = Layers[i_layer].prod_lambdas.dot(I_prod);  // coeff for dHs

  I_prod = I_ijk * Layers[i_layer].lambdas;
  cdJ_diag[1] = Layers[i_layer].lambdas.dot(I_prod);        // coeff for H*dH*H
  cdJ_off[0]  = Layers[i_layer].prod_lambdas.dot(I_prod);   // coeff for {dH,H}

  I_prod = I_ijk * Eigen::Vector3cd::Ones();
  cdJ_diag[2] = Eigen::Vector3cd::Ones().dot(I_prod);        // coeff for H²*dH*H²
  cdJ_off[1]  = Layers[i_layer].prod_lambdas.dot(I_prod);   // coeff for {dH,H²}
  cdJ_off[2]  = Layers[i_layer].lambdas.dot(I_prod);        // coeff for H*{dH,H}*H
  ...
  // J is already up to date from the preceding _layer_amplitude call
  if (is_deepest) {
    dJ = dJ_layer;
  } else {
    dJ = J_layer * dJ * J_layer + J_layer * J * dJ_layer + dJ_layer * J * J_layer;
  }
}
...
Eigen::Matrix3d CHICEARTHDIFF::compute_oscillations_derivatives(std::string_view param,
                                                                 double E, double cos_zenith,
                                                                 double h) {
  // FIX: detect param change independently of energy change
  if (param != param0) {
  ...
  // Recompute if energy changed, vacuum params changed, OR param changed
  if (E != E0 || update_vacuum || param_changed) {
    E0 = E;
    _compute_hamiltonians_and_anticommutators();
  }
